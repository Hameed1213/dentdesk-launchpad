  ...
  return (
    <section className="relative bg-background py-20 md:py-32 px-6 md:px-8 overflow-hidden">
      <div className="relative mx-auto max-w-6xl">
        ...
        {/* Bento grid: 3 cols desktop, varied row spans */}
        <div className="grid grid-cols-1 md:grid-cols-6 auto-rows-[minmax(0,1fr)] gap-5 lg:gap-6">
          {cells.map((cell, i) => {
            const Icon = cell.Icon;
            // Layout: row 1 (3 + 3), row 2 (2 + 2 + 2)
            const span =
              i === 0
                ? "md:col-span-3"
                : i === 1
            ...
            return (
              <Card
                key={i}
                className={`group relative overflow-hidden border border-neutral-100 bg-white/30 backdrop-blur-xl backdrop-saturate-150 p-8 flex flex-col rounded-2xl shadow-none transition-all duration-300 hover:bg-white/40 hover:-translate-y-1 hover:border-neutral-200 ${span}`}
              >
